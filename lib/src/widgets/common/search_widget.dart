...
class MySingleChoiceSearchState<T> extends State<SearchWidget<T>> {
  ...
  @override
  void didUpdateWidget(SearchWidget<T> oldWidget) {
    ...
    final position = RelativeRect.fromRect(
      Rect.fromPoints(
        textFieldRenderBox.localToGlobal(
          textFieldRenderBox.paintBounds.size.topLeft(Offset.zero),
          ancestor: overlay,
        ),
        textFieldRenderBox.localToGlobal(
          textFieldRenderBox.paintBounds.size.topRight(Offset.zero),
          ancestor: overlay,
        ),
      ),
...
class KeyboardVisibilityNotification {
  ...
  void onKeyboardEvent(dynamic arg) {
    ...
    _list.forEach((subscriber, s) {
      try {
        if (s.onChange != null) {
          s.onChange?.call(isKeyboardVisible);
        }
